...
class TestRelationship:
    ...
    ###########################################################
    ##              BEGIN RELATIONSHIP TESTS                   ##
    ###########################################################

    def test_a_job_has_a_description(self):
        '''
        Test if models exists trying to create an instance
        return:
        '''
        job = mixer.blend(Job)
        desctiption = mixer.blend(Description, jobs=job)
        #pytest.set_trace()
        assert job.pk == desctiption.jobs.pk, 'Should be the same key'
